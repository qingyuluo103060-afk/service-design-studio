...
本版本面向“可发给学生公网试用”的课堂测试场景。学生通过网页注册、登录后进入工作台，使用项目流程与 AI 方法助手。学生可自行接入大模型 API Key；课堂测试版默认把学生 Key 保存在本人浏览器本地，调用时临时提交给后端代理，不写入服务器账号文件。
...
- `DEEPSEEK_API_KEY` 等模型密钥：可选。教师可以配置默认模型；学生也可以在网页里填写个人 API Key。
...
1. 打开教师提供的网址。
2. 首次使用选择“注册”，填写姓名、班级、学号、密码。学号需为 9 位数字，格式为 `21XX17XXX`。
3. 后续选择“登录”，进入服务设计工作台。
4. 选择所在小组，填写项目主题、调研证据、需求与方案。
5. 在“AI 方法助手”填写个人 API Key、选择模型，输入任务说明，生成服务设计建议。
6. 通过“导出数据”保留小组阶段成果。
...
建议保留当前 Node 后端作为 API 层，未来只替换前端：

1. 账号接口保持：`/api/auth/register`、`/api/auth/login`、`/api/me`。
2. 项目数据接口保持：`/api/state`。
3. 模型代理接口保持：`/api/llm/chat`。
4. 微信小程序端使用本地安全存储保存学生个人 API Key；调用模型时仍临时提交给后端代理。
5. 正式版再补数据库、分组权限、管理员后台和密钥加密存储。
